#!/usr/bin/env python3
"""Tune a conservative FAKE threshold using only fake videos from val split."""
...
def main():
    parser = argparse.ArgumentParser(description="Tune FAKE threshold using fake-only videos.")
    ...
    parser.add_argument("--fake-recall", type=float, default=0.9, help="Target recall for FAKE class.")
    parser.add_argument("--out", default=str(base_dir / "artifacts" / "fake_threshold.json"))
    ...
    if not probs:
        raise SystemExit("No fake video probabilities computed.")

    probs_sorted = sorted(probs)
    idx = max(0, min(len(probs_sorted) - 1, int(len(probs_sorted) * (1 - args.fake_recall))))
    fake_threshold = probs_sorted[idx]

    summary = {
        "fake_threshold": fake_threshold,
        "fake_recall_target": args.fake_recall,
        "num_fake_videos": used,
        "min_prob": min(probs),
        "max_prob": max(probs),
    ...
    print("Fake-only threshold summary:")
